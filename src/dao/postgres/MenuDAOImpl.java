...
public class MenuDAOImpl implements MenuDAO{
    ...
    public Menu GetMenuByName(String name) {
        return (Menu) sessionFactory.getCurrentSession().createQuery("from Menu where menutitle = :name").setParameter("name", name).uniqueResult();
    ...
    public MenuItem GetMenuItemByName(String name) {
       return (MenuItem) sessionFactory.getCurrentSession().createQuery("from MenuItem where menuItemtitle = :name").setParameter("name", name).uniqueResult();
    } //Получить пункт Меню по названию 

    @Override
    public List<MenuItem> GetMenuItemListByMenuID(int ID) {
        return sessionFactory.getCurrentSession().createQuery("from MenuItem where menu.menuid = :id").setParameter("id", ID).list(); 
    ...
    public MenuAcces GetMenuAccesByName(String name) {
         return (MenuAcces) sessionFactory.getCurrentSession().createQuery("from MenuAcces where menuAccesTitle = :name").setParameter("name", name).uniqueResult();
